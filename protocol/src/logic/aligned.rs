...
impl<T, ToSizeOfType> Parcel for Aligned<T, ToSizeOfType>
    ...
    fn read(read: &mut Read,
        ...
        let inner_value = T::read(read, settings, hints)?;
        let value_size = inner_value.raw_bytes_ext(settings, hints).unwrap().len();
        let padding_size = calculate_padding(Self::align_to_bytes(), value_size);

        for _ in 0..padding_size {
            let padding_byte = u8::read(read, settings, &mut hint::Hints::default())?;
    ...
    fn write(&self,
        ...
        let unaligned_bytes = self.value.raw_bytes_ext(settings, hints)?;
